...
import { createTask, getTask, listTasks, updateTask, deleteTask, setPrismaForTesting, resetPrismaForTesting } from './task.repository';
import type { CreateTaskInput } from '../task.types';
import type { PrismaClient } from '../generated/prisma/client';

if (!process.env.DATABASE_URL) {
  throw new Error(
    'DATABASE_URL is required for the repository test suite. ' +
      'Set it to a SQLite file URL like file:./test.db (CI uses this; locally run `npm run db:deploy` from the server workspace first).'
  );
}
